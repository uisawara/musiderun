...
`Tools/musiderun/Create Settings JSON` で上記に近い既定 JSON を生成できます。
...
| `mirrorBranchPrefix` | string | `"musiderun/mirror"` | **（現在未使用）** ミラーは detached HEAD で同期するため専用ブランチを作成しません。後方互換のためフィールドのみ残存 |
| `defaultWorkingBranch` | string | `"main"` | **（現在未使用）** ブランチ自動回復ロジック廃止に伴い不要。後方互換のためフィールドのみ残存 |
...
| フィールド | 型 | 既定値 | 説明 |
| ---------- | -- | ------ | ---- |
| `id` | string | `""` | Job の識別子。worktree パス・ログファイル名に使用 |
| `displayName` | string | `""` | ウィンドウに表示する名前 |
| `targetOS` | string | `"Any"` | 実行対象 OS。`Any` \| `Windows` \| `macOS`。現在の OS と一致しない Job はウィンドウで実行対象外 |
